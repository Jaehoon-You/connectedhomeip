...
static constexpr size_t kPowerSourceManagerTableSize = EMBER_AF_POWER_SOURCE_CLUSTER_SERVER_ENDPOINT_COUNT + CHIP_DEVICE_CONFIG_DYNAMIC_ENDPOINT_COUNT;
...
void PowerSourceManager::NewManager(jint endpoint, jobject manager)
{
    ...
    uint16_t ep = emberAfGetClusterServerEndpointIndex(static_cast<chip::EndpointId>(endpoint), app::Clusters::PowerSource::Id, EMBER_AF_POWER_SOURCE_CLUSTER_SERVER_ENDPOINT_COUNT);
...
{
    uint16_t ep = emberAfGetClusterServerEndpointIndex(endpoint, app::Clusters::PowerSource::Id, EMBER_AF_POWER_SOURCE_CLUSTER_SERVER_ENDPOINT_COUNT);
...
jboolean PowerSourceManager::SetBatPercentRemaining(jint endpoint, jint value)
{
    ...
    status = PowerSource::Attributes::BatPercentRemaining::Set(static_cast<chip::EndpointId>(endpoint),
